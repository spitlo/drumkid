...
int updateAudio() {
  ...
  char asig = ((sampleVolumes[0] * sample0.next()) >> atten) + ((sampleVolumes[1] * sample1.next()) >> atten) + ((sampleVolumes[2] * sample2.next()) >> atten) + ((sampleVolumes[3] * sample3.next()) >> atten) + ((sampleVolumes[4] * sample4.next()) >> atten) + (droneSig >> 2);
...
void createRandomSession() {
  ...
  // Don't randomise time signature, tempo or swing unless stopped
  if (!beatPlaying) {
    storedValues[TIME_SIGNATURE] = 64;  // equates to 4/4
